...
```{r}
# Subset to switch mice
dds_switch_liver_male <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Liver" & dds$Sex =="Male"],design = ~ group)

# Relevel to set GFP Control as the reference group
dds_switch_liver_male$group = relevel(dds_switch_liver_male$group, ref = "GFP Control")

# Run DESeq2
des_switch_liver_male <- DESeq(dds_switch_liver_male)

# Check results names
# resultsNames(des_switch_liver_male)

# Get results
res_liver_male <- results(des_switch_liver_male) |> as.data.frame()

significant_genes_male <- res_liver_male |> 
  rownames_to_column(var = "gene") |> 
  filter(padj < 0.01 & abs(log2FoldChange) > 1)
```

There are `r nrow(significant_genes_male)` significantly differentially expressed genes (FDR \< 0.01, \|log2FC\| \> 1) in the liver between Cre-AAV and GFP control switch mice.
...
p <- ggplot(res_liver_male,aes(pvalue)) +
  geom_histogram(binwidth = 0.05, fill = "lightblue", color = "darkblue") +
  labs(
     title = "Computed p-values\n for all genes",
     x = "\np-value (Wald test)")+
...
b <- ggplot(significant_genes_male,aes(log(baseMean+1))) +
  geom_histogram( fill = "lightblue", color = "darkblue") +
  labs(
     title = "Log baseMean for all significant genes",
     x = "\nLog10 (baseMean + 1)")+
...
```{r}
# Subset to switch mice
dds_switch_liver_female <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Liver" & dds$Sex =="Female"],design = ~ group)

# Relevel to set GFP Control as the reference group
dds_switch_liver_female$group = relevel(dds_switch_liver_female$group, ref = "GFP Control")

# Run DESeq2
des_switch_liver_female <- DESeq(dds_switch_liver_female)

# Check results names
# resultsNames(des_switch_liver_female)

# Get results
res_liver_female <- results(des_switch_liver_female) |> as.data.frame()

significant_genes_female <- res_liver_female |> 
  rownames_to_column(var = "gene") |> 
  filter(padj < 0.01 & abs(log2FoldChange) > 1)
```

There are `r nrow(significant_genes_female)` significantly differentially expressed genes (FDR \< 0.01, \|log2FC\| \> 1) in the liver between **female** Cre-AAV and GFP control switch mice.
...
p <- ggplot(res_liver_female,aes(pvalue)) +
  geom_histogram(binwidth = 0.05, fill = "lightblue", color = "darkblue") +
  labs(
     title = "Computed p-values\n for all genes",
     x = "\np-value (Wald test)")+
...
b <- ggplot(significant_genes_female,aes(log(baseMean+1))) +
  geom_histogram( fill = "lightblue", color = "darkblue") +
  labs(
     title = "Log baseMean for all significant genes",
     x = "\nLog10 (baseMean + 1)")+
...
#### Common DEGs in males and females
...
```{r}
common_degs <- intersect(significant_genes_male$gene, significant_genes_female$gene)
common_degs_df <- data.frame(gene = common_degs)  
```

There were `r nrow(common_degs_df)` common DEGs shared in male and females. The majority corresponded to ribosomal and uncharacterized pseudogenes, reflecting global transcriptional perturbations rather than specific pathway activation.  
...
```{r}
genes <- common_degs_df |> 
  filter(!str_detect(gene,"^Rp|^Gm|^mt|ENSMUS|-ps")) 
```

Filtering out pseudogenes, unannotated loci (Gm*), mitochondrial genes, and Ensembl-only identifiers yielded `r nrow(genes)` high-confidence nuclear genes consistently dysregulated across sexes, representing the shared hepatic response to Cre-mediated ApoE4→ApoE2 conversion.

```{r}
cat(paste(genes$gene, collapse = ", "))
```
...
```{r}
# Subset to switch mice
dds_switch_brain_male <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Brain" & dds$Sex =="Male"],design = ~ group)

# Relevel to set GFP Control as the reference group
dds_switch_brain_male$group = relevel(dds_switch_brain_male$group, ref = "GFP Control")

# Run DESeq2
des_switch_brain_male <- DESeq(dds_switch_brain_male)

# Check results names
# resultsNames(des_switch_brain_male)

# Get results
res_brain_male <- results(des_switch_brain_male) |> as.data.frame()

significant_genes_male_brain <- res_brain_male |> 
  rownames_to_column(var = "gene") |> 
  filter(padj < 0.01 & abs(log2FoldChange) > 1)
```

There are `r nrow(significant_genes_male_brain)` significantly differentially expressed genes (FDR \< 0.01, \|log2FC\| \> 1) in the brain between Cre-AAV and GFP control switch mice.
...
p <- ggplot(res_brain_male,aes(pvalue)) +
  geom_histogram(binwidth = 0.05, fill = "lightblue", color = "darkblue") +
  labs(
     title = "Computed p-values\n for all genes",
     x = "\np-value (Wald test)")+
...
b <- ggplot(significant_genes_male_brain,aes(log(baseMean+1))) +
  geom_histogram( fill = "lightblue", color = "darkblue") +
  labs(
     title = "Log baseMean for all significant genes",
     x = "\nLog10 (baseMean + 1)")+
...
```{r}
# Subset to switch mice
dds_switch_brain_female <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Brain" & dds$Sex =="Female"],design = ~ group)

# Relevel to set GFP Control as the reference group
dds_switch_brain_female$group = relevel(dds_switch_brain_female$group, ref = "GFP Control")

# Run DESeq2
des_switch_brain_female <- DESeq(dds_switch_brain_female)

# Check results names
# resultsNames(des_switch_brain_female)

# Get results
res_brain_female <- results(des_switch_brain_female) |> as.data.frame()

significant_genes_female_brain <- res_brain_female |> 
  rownames_to_column(var = "gene") |> 
  filter(padj < 0.01 & abs(log2FoldChange) > 1)
```

There are `r nrow(significant_genes_female_brain)` significantly differentially expressed genes (FDR \< 0.01, \|log2FC\| \> 1) in the brain between **female** Cre-AAV and GFP control switch mice.
...
p <- ggplot(res_brain_female,aes(pvalue)) +
  geom_histogram(binwidth = 0.05, fill = "lightblue", color = "darkblue") +
  labs(
     title = "Computed p-values\n for all genes",
     x = "\np-value (Wald test)")+
...
b <- ggplot(significant_genes_female_brain,aes(log(baseMean+1))) +
  geom_histogram( fill = "lightblue", color = "darkblue") +
  labs(
     title = "Log baseMean for all significant genes",
     x = "\nLog10 (baseMean + 1)")+
...
# Display significant genes in a table
significant_genes_female_brain |> 
  select(gene,baseMean, log2FoldChange, padj) |>
  arrange(padj) |> 
  kable(digits = 2) |> 
  kable_styling(full_width = F) |> 
...
```{r}
common_degs_brain <- intersect(significant_genes_male_brain$gene, significant_genes_female_brain$gene)
common_degs_brain_df <- data.frame(gene = common_degs_brain)  
```

There were `r nrow(common_degs_brain_df)` common DEGs shared in male and females. The majority corresponded to ribosomal and uncharacterized pseudogenes, reflecting global transcriptional perturbations rather than specific pathway activation.  

```{r}

# Display significant genes in a table
common_degs_brain_df |> 
  kable(digits = 2) |> 
  kable_styling(full_width = F) |> 
  scroll_box(width = "100%", height = "400px")
...
```{r}
genes_brain <- common_degs_brain_df |> 
  filter(!str_detect(gene,"^Rp|^Gm|^mt|ENSMUS|-ps")) 
```

Filtering out pseudogenes, unannotated loci (Gm*), mitochondrial genes, and Ensembl-only identifiers yielded `r nrow(genes_brain)` high-confidence nuclear genes consistently dysregulated across sexes, representing the shared brain response to Cre-mediated ApoE4→ApoE2 conversion.

```{r}
cat(paste(genes_brain$gene, collapse = ", "))
```
