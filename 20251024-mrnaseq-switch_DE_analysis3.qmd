...
> Only samples from batch 2 are included in this analysis to avoid confounding batch effects.
...
```{r}
# Subset to switch mice
dds_switch_liver_male <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Liver" & dds$Sex =="Male" & dds$batch == "2"],
design = ~ group)
...
```{r}
# Subset to switch mice
dds_switch_liver_female <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Liver" & dds$Sex =="Female" & dds$batch == "2"],
design = ~ group)
...
There were `r nrow(common_degs_df)` common DEGs shared in male and females. 
...
Significance with FDR \< 0.01. No FC cutoff.
...
##### **Significantly differentially expressed genes in brain**<br>Cre-AAV and GFP control <br>(FDR \< 0.05) {.unnumbered}
...
```{r}
# Subset to switch mice
dds_switch_brain_male <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Brain" & dds$Sex =="Male" & dds$batch=="2"],design = ~ group)
...
##### **Significantly differentially expressed genes in brain**<br>Cre-AAV and GFP control <br>(FDR \< 0.01) {.unnumbered}
...
```{r}
# Subset to switch mice
dds_switch_brain_female <- DESeqDataSet(dds[,dds$mice == "switch" & dds$Tissue == "Brain" & dds$Sex =="Female" & dds$batch == "2"],design = ~ group)
...
There were 0 common DEGs shared in male and females. 
